...
class RenamerApp(RenamerAppV9):
    ...
    custom MkPFS selection and a negative cache so unchanged problematic images
    do not repeatedly pay the MkPFS parsing cost on every scan.
    """
    ...
    def _snapshot_settings(self) -> AppSettings:
        return replace(super()._snapshot_settings(), mkpfs_path=self._mkpfs_path)
    ...
    def _update_in_memory_paths(self, mapping: dict[Path, Path]) -> None:
        ...
        for item in self.partial_items:
            ...
            updated_partial.append(
                (mapped(path), metadata, detail, inference_source, code, friendly)
            )
