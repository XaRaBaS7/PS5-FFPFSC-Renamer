...
class ActivityProgressMixin:
    """Activity logging plus dual-level scan progress."""
    ...
    def _scan(self) -> None:
        roots = self.library_roots or (
            [Path(self.folder_var.get().strip())] if self.folder_var.get().strip() else []
        )
        if roots:
            self._log(
                "INFO",
    ...
    def _cache_check_progress(self, checked: int, total: int, hits: int, new: int) -> None:
        ...
        if self._activity_text is not None:
            self._activity_text.set(
                f"Cache check {checked}/{total} • hits {hits} • new/changed {new}"
            )
        if self._overall_progress_text is not None:
        ...
        *,
        ...
        if completed:
            ...
            if len(completed) > 20:
                self._log(
                    "INFO",
                    f"Rename log abbreviated: {len(completed) - 20} additional item(s)",
                )
