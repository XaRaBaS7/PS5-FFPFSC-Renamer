...
class LiveWatchMixin:
    """Optional low-impact library change watching."""
    ...
    def _watch_result(self, snapshot: LibrarySnapshot) -> None:
        ...
        if self._watch_status_var is not None:
            self._watch_status_var.set("Change detected • refreshing library")
    ...
    def _find_notebook(widget: tk.Misc) -> ttk.Notebook | None:
        for child in widget.winfo_children():
            if isinstance(child, ttk.Notebook):
                return child
            nested = LiveWatchMixin._find_notebook(child)
