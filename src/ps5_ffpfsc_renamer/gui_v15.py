...
from .gui_v14 import RenamerApp as RenamerAppV14
from .library_view import human_size
from .naming import FOLDER_ALWAYS_NEW, FOLDER_FILE_ONLY, FOLDER_SMART
...
class RenamerApp(RenamerAppV14):
    """v0.4 naming profiles, separators and details-cache maintenance."""
    ...
    @staticmethod
    def _folder_label(mode: str) -> str:
        if mode == FOLDER_FILE_ONLY:
            return RenamerApp.FOLDER_FILE_ONLY_LABEL
        if mode == FOLDER_ALWAYS_NEW:
            return RenamerApp.FOLDER_ALWAYS_NEW_LABEL
        return RenamerApp.FOLDER_SMART_LABEL
    ...
    def _show_naming_profiles(self) -> None:
        ...
        def show_selected(_event=None) -> None:
            ...
            active_order = [order_names[item] for item in profile.component_order if order_names[item] in enabled]
            detail_var.set(
                f"{profile.name}\n\n"
    ...
    def _show_cache_manager(self) -> None:
        ...
        def prune() -> None:
            ...
            self._log("CACHE", f"Details cache pruned: {removed} stale entr{'y' if removed == 1 else 'ies'} removed")
        ...
        def clear() -> None:
            ...
            refresh_details_stats()
            self._log("CACHE", f"Details cache cleared: {removed} entr{'y' if removed == 1 else 'ies'} removed")
